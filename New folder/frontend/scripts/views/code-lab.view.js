...
export function codeLab({ value, output, solution, taskBrief = null, previousDisabled, nextDisabled = false, nextLabel, feedback = "", feedbackType = "info" }) {
  const theoryTask = isTheoryTask(taskBrief);
  const editorValue = theoryTask && value.trim() === "// Viết code của bạn ở đây"
    ? "// Viết câu trả lời của bạn tại đây\n"
    : value;
  const outputLabel = theoryTask ? "CÂU TRẢ LỜI MONG ĐỢI" : "OUTPUT MONG ĐỢI";
  const outputInstruction = theoryTask
    ? "Trả lời bằng comment bắt đầu với <code>//</code>. Bài lý thuyết không bắt buộc có output trong Console."
    : "In kết quả theo yêu cầu bằng <code>console.log()</code>. Kết quả chạy sẽ hiện ở Console bên dưới.";
  ...
  return `
    <section class="code-lab card h-100" aria-label="Code Lab">
      ...
      <textarea id="codeEditor" class="code-editor form-control" spellcheck="false">${escape(editorValue)}</textarea>
      ...
      ${feedback ? `
        <div class="alert alert-${escape(feedbackType)} code-feedback" role="alert" aria-live="assertive">
          ${escape(feedback)}
        </div>
      ` : ""}
